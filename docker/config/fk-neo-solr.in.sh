...
GC_TUNE=" -XX:+UseG1GC -XX:+PerfDisableSharedMem -XX:+ParallelRefProcEnabled -XX:G1HeapRegionSize=8m -XX:MaxGCPauseMillis=60 -XX:+AggressiveOpts "


# Set the ZooKeeper connection string if using an external ZooKeeper ensemble
# e.g. host1:2181,host2:2181/chroot
# Leave empty if not using SolrCloud
ZK_HOST=zookeeper:2181/docker-solr-a
...
# For remote debugging (not recommended in prod).
#SOLR_OPTS="$SOLR_OPTS -Xdebug -Xrunjdwp:transport=dt_socket,server=y,suspend=n,address=4044"

# Location where the bin/solr script will save PID files for running instances
# If not set, the script will create PID files in $SOLR_TIP/bin
SOLR_PID_DIR="/var/fk-neo-solr"

# Path to a directory for Solr to store cores and their data. By default, Solr will use server/solr
# If solr.xml is not stored in ZooKeeper, this directory needs to contain solr.xml
SOLR_HOME="/var/fk-neo-solr/data"
...
LOG4J_PROPS=/var/fk-neo-solr/log4j2.xml
...
SOLR_LOGS_DIR="/var/fk-neo-solr/logs"
